...
app.use('/api', async (req, res, next) => {
    ...
    if (!authHeader || !authHeader.startsWith('Bearer ')) {
      return res.status(401).json({
        code: 401,
        data: null,
        message: "缺少有效的 Authorization 头部",
      });
    }
    ...
    if (users.length === 0 || users[0].status == 'Ban') {//检查用户是否被封禁,限制封禁用户的资源请求.
      return res.status(403).json({//当状态不对时返回信息,账户已被禁用或不存在
        code: 403,
        data: null,
        message: "账户已被禁用或不存在"
      });
    }
    req.user = { ...decoded, role: users[0].role,status:users[0].status };//将用户信息附加到请求对象中
    next();//通过
    } catch (err) {//错误处理
      return res.status(401).json({
        code: 401,
...
app.use('/api',(req, res, next) => {
    if (req.url === '/client/register' || req.url === '/client/login'||req.url === '/admin/login') {//跳过登录注册
      return next();
    }
  ...
  res.json = function (data) {
    ...
    originalJson.call(this, {
      code: data.code||200,
      data: data.data||null,
      message: data.message+`操作成功${req.url}`,
      userStatus:{
        id: req.user.userId,
        role: req.user.role,
